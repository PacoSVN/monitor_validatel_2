...
module.exports.validarUsuario = async(datos, event) => {
    ...
    if (usuario.length > 0) {
        ...
        if (nuevoUsurio.SSUSRPSW == "") {
            ...
            var url = datos.url.valor + "?psw=" + datos.pssw + "&llave=" + key + "&pswdb=" + nuevoUsurio.SSUSRPSW;
            console.log(url);
...
module.exports.consultarUrlPass = async(version) => {

    const urlPass = await pool.query(querys.ConsultarUrlPass, [version]); 
    return urlPass[0];
}