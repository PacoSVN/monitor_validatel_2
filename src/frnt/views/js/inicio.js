...
ipcRenderer.on('consultarSupervisoresResult', (event, datos) => {
    ...
    // parece que no se ocupa MARISOL conectarSocket();
})
...
ipcRenderer.on('getConsultarAgenteOResult', (event, datos) => {
...
function total_indicadores() {
    ...
    for (let a = 0; a < 2; a++) {
        ...
        var separador = ".";
        var separador2 = " - ";
        if(!arrConexiones[a].includes("0.0.0.")){
...
ipcRenderer.on('getIdnAgntsResult', (event, datos) => {
   ...
   /* var datos_={};
    ...
    if (datos_.AHT == "null" || datos_.AHT == null) {
         $("#lblaht").text("00:00:00");
    } else {
        if (datos_.AHT != '00:00:00') 
        {
        ...
        for (let i = 0; i < preferencias.length; i++) {
            ...
            if(preferencias[i].dsc==2)
            {
                for(var a = 0; a >= arrConexiones.length; a++)
                {
                    var arregloCadena1 = [];
                    var separador = ".";
                    //var tmparregloCadena1 = arrConexiones[a];
                    arregloCadena1 = arrConexiones[a].split(separador);
            ...
            if (preferencias[i].dsc == 1 && numeromarcadores == 2) {
                for(var x= 0; x >= arrConexiones.length; x++)
                {
                    var arregloCadena1 = [];
                    var separador = ".";
                    //var tmparregloCadena1 = arrConexiones[i];
                    arregloCadena1 = arrConexiones[i].split(separador);
...
ipcRenderer.on('getindicadoresAgentesResult',  async(event, dato) => {
    ...
    for (let x = 0; x < dato.detalle.length; x++) {
            var areaPintarinb= dato.detalle[x].area;
            if(areaPintarinb=='I'){
  ...
  function consultarIndicadoresReferencia(campana, cola, supervisor, sts, stsrec, idagnt, nombre)
  {
